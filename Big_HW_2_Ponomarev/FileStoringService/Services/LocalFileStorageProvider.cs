...
namespace FileStoringService.Services
{
    public class LocalFileStorageProvider : IFileStorageProvider
    {
        ...
        {
            _logger = logger;
            _basePath = settings.Value.BasePath ?? throw new ArgumentNullException(nameof(settings.Value.BasePath), "File storage base path is not configured.");
        ...
        public async Task<string> SaveFileAsync(Stream stream, string originalFileName, string contentType)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Successfully saved file: {StoredFileName} from original: {OriginalFileName}", storedFileName, originalFileName);
                return storedFileName;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to save file: {OriginalFileName} to path: {FilePath}", originalFileName, filePath);
                throw; 
            }
        }
