...
public class FileStoringServiceClient : IFileStoringServiceClient
{
    ...
    public async Task<FileData?> GetFileDataAsync(Guid fileId)
        {
            ...
            try
            {
                _logger.LogDebug("Requesting metadata from: {BaseAddress}{MetadataUrl}", _httpClient.BaseAddress, metadataUrl);
            ...
            {
                _logger.LogWarning("Failed to get metadata from FileStoringService for FileId: {FileId}. Status: {StatusCode}", fileId, metadataResponse.StatusCode);
                return null;
            }
            ...
            try
            {
                 _logger.LogDebug("Requesting file content from: {BaseAddress}{DownloadUrl}", _httpClient.BaseAddress, downloadUrl);
            ...
            {
                _logger.LogWarning("Failed to download file from FileStoringService for FileId: {FileId}. Status: {StatusCode}", fileId, fileResponse.StatusCode);
                return null;
            }
        ...
        ILogger<FileStoringServiceClient> logger)
    {
        ...
        _fileStoringServiceBaseUrl = configuration["ServiceUrls:FileStoringService"]
                                     ?? throw new InvalidOperationException("FileStoringService URL is not configured in ServiceUrls:FileStoringService.");
        if (_httpClient.BaseAddress == null && !string.IsNullOrEmpty(_fileStoringServiceBaseUrl))
        {
            _httpClient.BaseAddress = new Uri(_fileStoringServiceBaseUrl.EndsWith('/') ? _fileStoringServiceBaseUrl : _fileStoringServiceBaseUrl + "/");
        }
        else if (_httpClient.BaseAddress == null)
        {
            _logger.LogError("HttpClient BaseAddress is not set and FileStoringService URL is missing in configuration.");
        }
    }
}